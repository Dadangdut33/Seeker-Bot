...
const slashCommands: ISlashCommand = {
	...
	execute: async (interaction) => {
		...
		const embed = new EmbedBuilder()
			.setColor("Random")
			.setFooter({ text: description ? `${description} Spoiler Above!!` : interaction.user.username })
			.setTitle(`SPOILER WARNING!!${description ? `\`${description}\`` : ``} SPOILER ABOVE!!`)
