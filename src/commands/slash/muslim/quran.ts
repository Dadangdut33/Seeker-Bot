...
const slashCommands: ISlashCommand = {
		...
		try {
			...
			if (subcommand === "surah") {
				...
				for (let i = 0; i < choices.length; i++) {
					...
					if (choice.name.toLowerCase().includes(focusedValue.value.toLowerCase())) filtered.push(choice);
				}
	...
	execute: async (interaction) => {
		...
		if (command === "surah") {
			...
			interactionBtnPaginator(interaction, data, 120, { components_function: component_func }); // 120 minutes
		} else {
