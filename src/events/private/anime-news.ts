...
import { send_crunchyroll, send_mal } from "../../utils/rss";
...
const event: IBotEvent = {
	...
	execute: async (client: Client) => {
		...
		if (!gid || !mal_ch_id || !crunchyroll_ch_id) return logger.warn("guild or channel ID not set!");
		...
		if (!mal_ch) return logger.warn("Invalid channel for Nyaa rss feed");

		const crunchyroll_ch = theGuild.channels.cache.get(crunchyroll_ch_id) as TextChannel;
		if (!crunchyroll_ch) return logger.warn("Invalid channel for Nyaa rss feed");
